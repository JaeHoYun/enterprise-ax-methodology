...
| **회수(repatriation)** | 퍼블릭 클라우드로 나갔던 AI 워크로드를 온프레미스·사내로 되돌리는 것. 지속 가동률·회수기간·사용 패턴·데이터 주권으로 판정 |
| **데이터 중력(data gravity)** | 데이터가 쌓일수록 옮기기 어려워지고 연산이 데이터 쪽으로 끌려오는 경향. 회수·배치 결정의 한 축 |
...
| **자산 처분(6R)** | 현존 자산의 처리 결정 — 잔류·이전·회수·통합·재구성·폐기. 민감도·단가·재작성비·종속도로 스코어링 |
| **매니지드/서버리스(managed/serverless)** | 인프라 운영을 외부 사업자에게 맡기는 소비 형태. 매니지드=관리형 서비스로 운영 부담을 위임, 서버리스=요청 단위 과금으로 유휴 비용을 없앰. 회수 판정의 앞단에서 현행 유지가 기본값인 자산 유형 |
