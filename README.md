...
| 03 | [대안 — 증거 기반 점진적 AX](docs/03-alternative.md) | 6대 원칙(업무 문제에서 역산, 최소 관통 파일럿, 게이트, 결정의 되돌림 가능성 구분, 플라이휠, 수요 확인 후 플랫폼 구축), 실행 루프 |
| 04 | [유스케이스 발굴, 우선순위, 가치 가설](docs/04-portfolio.md) | 발굴법, 가치 가설 캔버스, 우선순위 매트릭스, 결과가 불확실한 과제의 가치 평가, 포트폴리오, 투자 케이스(가치와 비용의 연결) |
| 05 | [운영모델, 조직, 거버넌스](docs/05-operating-model.md) | 중앙, 연합, 분산 운영모델, 역할, 인재와 사용 정착 설계, 비례적 거버넌스 |
| 06 | [전사 확장 — 규모의 아키텍처와 거버넌스](docs/06-enterprise-scale.md) | 확장 전환 신호, 진화형 아키텍처(수요에 따라 쌓는 계층 구조), 골든 패스, 데이터와 지식, 거버넌스, FinOps, 신뢰성, 확장기의 흔한 실수 |
| 07 | [Private AI로 AX 갖추기](docs/07-private-ai-strategy.md) | 왜, 언제 Private AI인가, 단계적 전환, 빌딩블록, 수요 확인 후 플랫폼 구축과 도입 타이밍 |
...
| A1 | [FAQ, 참고](appendix/A1-appendix.md) | 자주 묻는 질문, 참고 자료 |
...
| **업무 문제에서 역산** | 기술이 아니라 구체적 업무 페인에서 출발해 필요한 기술을 거꾸로 짚는 원칙 |
| **최소 관통 파일럿** | 한 업무를 처음부터 끝까지 관통하되 범위는 최소로 좁힌 파일럿 |
| **게이트** | 파일럿을 사전 기준으로 확장/중단 판정하는 관문 |
| **결정의 되돌림 가능성 구분** | 되돌릴 수 있는 결정은 빠르게, 되돌리기 어려운 결정은 증거 축적 후 |
| **역량 플라이휠** | 파일럿마다 재사용 자산을 쌓아 다음 시도를 싸게 만드는 선순환 |
| **수요 확인 후 플랫폼 구축** | 플랫폼을 미리 깔지 않고, 여러 파일럿의 공통 수요가 드러날 때 공통화하는 원칙 |
| **Private AI** | 데이터가 조직 경계를 벗어나지 않은 채 사내에서 운영하는 AI |

그 밖의 용어는 각 문서에서 처음 나오는 절에서 정의합니다.
